...
export function StaticScreen({ cluster=false, palette=false, canvasId="StaticCanvas" }) {
  ...
  const staticControlBar = (
    <div className="controlBar">
      ...
      />
      <MdRepeat onClick={() => viewRef.current.reset()} className="iconBtn" />
    </div>
  );
  ...
  if (loadStatus === null) {
    return <div style={styles.loading}>loading data ⌛️</div>;
  } else {
    return (
      <>
        {staticControlBar}
...
const CloudMeshDynamic = ({ temporalMesh, innerRef }) => {
  ...
  return <RayGrab>{pointMesh}</RayGrab>;
};
